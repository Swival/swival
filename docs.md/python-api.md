...
Invalid configuration before the agent loop starts: missing model, missing API key, malformed MCP server config, non-existent directories in `allowed_dirs`, conflicting options.
