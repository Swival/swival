...
def _resolve_paths(config: dict, config_dir: Path, source: str = "") -> None:
    ...
    # Profile bodies are overlaid verbatim later, so resolve their command
    # models here too.
    profiles = config.get("profiles")
    if isinstance(profiles, dict):
        for body in profiles.values():
            if isinstance(body, dict):
                _resolve_command_model(body, config_dir, source)
...
def _load_single(path: Path, label: str) -> dict:
    ...
    if encrypt_patterns is not None:
        ...
        for i, pat in enumerate(encrypt_patterns):
            ...
            for req in ("name", "prefix", "body_regex"):
                if req not in pat:
                    raise ConfigError(
                        f"{label}: encrypt_secrets_patterns[{i}]: missing required key {req!r}"
                    )
                if not isinstance(pat[req], str):
                    raise ConfigError(
                        f"{label}: encrypt_secrets_patterns[{i}]: {req!r} must be a string"
                    )
...
def _extract_raw_tables(raw: str, keys_present: set[str]) -> tuple[str, set[str]]:
    """Extract nested table blocks from raw TOML text for the given root keys.

    Also returns which keys appeared as table headers; keys written inline
    have none and must be regenerated by the caller.
    """
    collected: list[str] = []
...
def generate_config(
    ...
    *existing_raw* and appended after all scalar keys.
    ...
    template_lines = [
        ...
        "# --- Features ---",
        "# no_instructions = false",
        "# instructions_full = false      # load all instruction files whatever their size",
        "#   An explicit --no-instructions wins over an inherited full-loading setting;",
        "#   reject contradictory explicit CLI flags.",
        "# no_skills = false",
