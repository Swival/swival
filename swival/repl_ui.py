...
_TICK_SECONDS = 1 / 12
_FLASH_SECONDS = 3.0
# How long the main thread waits for the UI thread to give up the terminal.
# Only a UI thread stuck inside prompt_toolkit can exhaust it: a thread that
# exits for any reason releases the wait on its way out.
_PARK_TIMEOUT = 10.0
...
class _Stop:
    """Result the prompt returns when the session is shutting down."""


class _Suspend:
    """Result the prompt returns when the main thread needs the terminal."""
...
class _OutputRouter:
    ...
    def write(self, channel: str, data: str) -> None:
        ...
        with self._lock:
            ...
            if self._app is None:
                # Nothing is drawn below the cursor, so partial lines can go
                # out as they are.
    ...
    def nudge(self) -> None:
        ...
        try:
            loop.call_soon_threadsafe(self._drain_in_loop, context=context)
        except RuntimeError:
            # The loop closed between the check and the call. The prompt is
            # going away, and prompt_finished() drains whatever is left.
            with self._lock:
                self._scheduled = False
...
class _SpinnerSlot(_Slot):
    ...
    def __init__(self, label: str, *, phases: bool = True) -> None:
        super().__init__()
        self.label = label
        self.phases = phases
        self.suffix = ""
        if phases and "(" in label:
            idx = label.index("(")
            self.suffix = " " + label[idx:].strip()

    def _phase(self) -> tuple[str, str, str]:
        if not self.phases:
            return "dots", "cyan", self.label
        elapsed = self.elapsed
        current = fmt._SPINNER_PHASES[0]
        for phase in fmt._SPINNER_PHASES:
            if elapsed >= phase[0]:
                current = phase
        _, name, style, verb = current
        return name, style, f"{verb}{self.suffix}"

    def render(self, width: int, budget: int):
        name, style, desc = self._phase()
        text = Text("  ")
        text.append_text(Spinner(name, style=style, speed=1.5).render(self.elapsed))
        text.append(f"  {desc}", style=style)
        text.append(f"  {_format_elapsed(self.elapsed)}", style="dim")
        return text
...
class _StepSlot(_Slot):
    ...
    def render(self, width: int, budget: int):
        text = Text("  ")
        text.append_text(Spinner("dots", style="cyan", speed=1.5).render(self.elapsed))
        text.append(f"  {self.label}", style="cyan")
        text.append(f"  {_format_elapsed(self.elapsed)}", style="dim")
        return text
...
class _CommandSlot(_Slot):
    ...
    def render(self, width: int, budget: int):
        ...
        grid.add_column(no_wrap=True)
        cells.append(Text(_format_elapsed(self.elapsed), style="dim"))
        grid.add_row(*cells)
        return grid
...
class _ReplLiveBackend:
    """Adapter that lets ``fmt`` render its transient displays in the UI."""

    def __init__(self, ui: "ReplUI") -> None:
        self._ui = ui

    @contextlib.contextmanager
    def _slot(self, slot: _Slot, handle):
        self._ui.add_slot(slot)
    ...
    def llm_spinner(self, label: str):
        return self._slot(_SpinnerSlot(label), None)

    def command_spinner(self, label: str, timeout: float | None):
        return self._slot(_CommandSlot(label, timeout), None)
    ...
    def input_marquee(self, text: str):
        return self._slot(_MarqueeSlot(text, None, 0.0), None)

    def input_marquee_then_spinner(self, text: str, spinner_label: str, delay: float):
        return self._slot(_MarqueeSlot(text, spinner_label, delay), None)
...
class ReplUI:
    ...
    with :meth:`next_event` from the main thread. Wrap each turn in
    :meth:`set_busy` so Ctrl-C and queueing know a turn is running.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._styles: list = []
        self._idle_style = None

        self._router = _OutputRouter()
        self._proxies_installed = False
        ...
        # Lines submitted while a turn was running, in order. These are the
        # ones an interrupt hands back to the editor.
        self._queued: list[ReplEvent] = []
        ...
        self.backend = _ReplLiveBackend(self)

    # Prompt session construction, main thread.
    ...
    def build_session(
        ...
        *,
        ...
        Imports stay local so tests can substitute scripted stand-ins for the
        prompt_toolkit pieces.
        """
        ...
        self._idle_style = Style.from_dict(_BASE_STYLE)
        self._styles = []
        for step in range(_BORDER_STEPS):
            t = step / (_BORDER_STEPS - 1)
            r, g, b = fmt._lerp_color(fmt._TURN_GRADIENT, t)
            self._styles.append(
                Style.from_dict(
                    {**_BASE_STYLE, "frame.border": f"#{r:02x}{g:02x}{b:02x}"}
                )
            )
    ...
    def _graft_status_region(self, session) -> None:
        ...
        session.style = DynamicStyle(self.current_style)
        app.min_redraw_interval = 0.02

    def current_style(self):
        phase = self.border_phase()
        if phase is None or not self._styles:
            return self._idle_style
        # Triangle wave through the gradient, so the border breathes instead
        # of snapping back to the first color.
        position = phase * 2
        if position > 1:
            position = 2 - position
        index = min(int(position * (len(self._styles) - 1)), len(self._styles) - 1)
    ...
    def prompt_message(self) -> list:
        return [(self.prompt_style(), _PROMPT_SYMBOL)]
    ...
    def _status_fragments(self) -> list:
        ...
        try:
            ...
            width, rows = 80, 24
        try:
            input_rows = session.default_buffer.document.line_count + 3
        except Exception:
            input_rows = 4
    ...
    def toolbar(self) -> list:
        ...
        return _with_right_hint(fragments, hint, self._terminal_width())
    ...
    def start(self) -> None:
        ...
        fmt.set_live_backend(self.backend)
    ...
    def stop(self) -> None:
        ...
        self._exit_prompt(_Stop)
        self._wake.set()
        if self._thread is not None:
            self._thread.join(timeout=_PARK_TIMEOUT)
            if self._thread.is_alive():
                self._warn_stuck("did not stop")
        if self._ticker is not None:
            self._ticker.join(timeout=1)
        if fmt.live_backend() is self.backend:
    ...
    def _install_proxies(self) -> None:
        ...
        self._proxies_installed = True

    def _uninstall_proxies(self) -> None:
        if not self._proxies_installed:
            return
        self._router.flush_partial()
        for channel, stream in self._original_streams.items():
            if channel == "stderr":
                sys.stderr = stream
            else:
                sys.stdout = stream
    ...
    def next_event(self, timeout: float | None = None) -> ReplEvent | None:
        ...
        if event.kind == "line":
            with self._lock:
                was_queued = event in self._queued
                if was_queued:
                    self._queued.remove(event)
            # A line submitted during a turn is echoed now, right above the
            # output of its own turn, rather than in the middle of the
            # previous one.
            if was_queued and fmt._console.is_terminal:
    ...
    def recall_queued(self) -> None:
        ...
        with self._lock:
            ...
            self._queued = []
        recalled = [e for e in queued if not _is_exit_command(e.text)]
        if not recalled:
            return
        with self._inbox_cv:
            for event in recalled:
                if event in self._inbox:
    ...
    def queued(self) -> list[str]:
        with self._lock:
            return [e.text for e in self._queued]
    ...
    # Status region slots, any thread.

    def add_slot(self, slot: _Slot) -> None:
        with self._lock:
    ...
    def refresh(self) -> None:
        app = getattr(self._session, "app", None)
        if app is None or not self._router.prompt_active:
            return
        try:
            app.invalidate()
        except Exception:
    ...
    def _tick(self) -> None:
        while not self._stopping:
            with self._lock:
                animated = bool(self._slots) or self._busy or self._flash is not None
            if animated:
                self.refresh()
    ...
    def _suspend_enter(self) -> None:
        with self._park_cv:
            self._suspend_count += 1
            first = self._suspend_count == 1
        if not first:
            return
        self._exit_prompt(_Suspend)
        if self._thread is not None and self._thread.is_alive():
            if not self._parked.wait(timeout=_PARK_TIMEOUT):
                self._warn_stuck("did not release the terminal")
        self._uninstall_proxies()

    def _suspend_exit(self) -> None:
        with self._park_cv:
            self._suspend_count -= 1
    ...
    def _exit_prompt(self, result) -> None:
        with self._router._lock:
            app = self._router._app
        if app is None:
            return
        ...
        self._call_in_prompt(_exit)

    # Key handling, UI thread.
        ...
        try:
            while not self._stopping:
                ...
                if _is_exit_command(text):
                    # Nothing more to read; the main loop exits once the
                    # current turn, if any, is over.
                    break
        finally:
            self._parked.set()
            self._emit_exit()
    ...
    def _on_pre_run(self) -> None:
        ...
        # A stop or suspend request that arrived between the previous prompt
        # ending and this one starting had no application to exit. Leave now.
        if self._stopping:
            app.exit(result=_Stop)
        elif self._suspend_count > 0:
    ...
    def _submit(self, text: str) -> None:
        if not text.strip():
            return
        event = ReplEvent("line", text)
        with self._lock:
            busy = self._busy
            if busy:
                self._queued.append(event)
        if not busy and fmt._console.is_terminal:
    ...
    # Rendering helpers, UI thread.

    def live_visible(self) -> bool:
        with self._lock:
            return bool(self._slots) or bool(self._queued)
    ...
    def border_phase(self) -> float | None:
        """Position in the breathing cycle while busy, ``None`` when idle."""
        if not self._busy:
            return None
        t = (time.monotonic() - self._busy_since) / 3.0
        return t - int(t)
...
def _with_right_hint(fragments: list, hint: str, width: int | None) -> list:
    ...
    try:
        ...
        used = fragment_list_width(fragments)
    except Exception:
        return fragments
    gap = width - used - len(hint) - 2
    if gap < 4:
        return fragments
...
def _make_shrink_wrap():
    from prompt_toolkit.layout.containers import Container
    from prompt_toolkit.layout.dimension import Dimension

    class ShrinkWrap(Container):
        """Give a container exactly its preferred height.

        The renderer hands the root layout every row below the cursor, and
        the framed input would grow to fill them. Capping the height at the
        preferred value keeps the box tight around the text while completion
        menus still get the space they reserve.
        """
...
def _ShrinkWrap(content):
    return _make_shrink_wrap()(content)
